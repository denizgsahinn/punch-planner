    ...
    <div className="min-h-screen bg-[#F7F3ED]">
      ...
      <div className="px-8 py-5">
        ...
        <div className="grid grid-cols-4 gap-3 mb-6">
          {[
            { label: "toplam proje",  value: String(projects?.length ?? 0) },
            { label: "devam ediyor",  value: String(projects?.filter(p => p.status === "wip").length ?? 0) },
            { label: "tamamlandı",    value: String(projects?.filter(p => p.status === "done").length ?? 0) },
            { label: "satışa hazır",  value: String(projects?.filter(p => p.status === "for_sale").length ?? 0) },
          ].map((s) => (
        ...
        {projects && projects.length > 0 ? (
            ...
            {projects.map((p: Project, i: number) => {
              ...
              return (
                <div key={p.id} className="bg-white rounded-[14px] border border-[#E4DBD4] overflow-hidden cursor-pointer hover:shadow-md transition-all duration-200 hover:-translate-y-0.5">
                  <div className={`h-44 bg-gradient-to-br ${bg} flex items-center justify-center relative`}>
                    {p.image_url ? (
          ...
          <div className="flex flex-col items-center justify-center mt-24 gap-3">
            ...
            <button className="mt-2 bg-[#8FAF8A] text-white text-sm px-5 py-2 rounded-full hover:bg-[#6B9A67] transition-colors">
              + Yeni Proje Ekle
            </button>
          </div>
        )}
